...
async function handleUpload(file) {
    ...
    try {
        // 🔄 MODIFIED: Ab direct remove.bg ki jagah hum apne Termux local backend par request bhej rahe hain
        const response = await fetch('http://localhost:3000/api/remove-bg', {
            method: 'POST',
            body: formData
            // Port aur Security Headers backend ke andar setup ho chuke hain automatic
        });
        ...
        console.error(err);
        alert("Could not connect to Termux Backend. Make sure 'node server.js' is running!"); 
        loadingSpinner.classList.add('hidden');
    }
}
